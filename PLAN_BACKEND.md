...
### 2.1 Auth 계약(권장 기본값)
- Signup 1-step (B안: user+dog 트랜잭션)
  - `POST /auth/signup` : 유저 + 강아지 생성 + 토큰 발급 (단일 트랜잭션)
  - 온보딩에서 별도 `POST /dogs` 호출 없이 완료
- Refresh
  - `POST /auth/refresh`
...
- `DogsController`
  ...
  - `GET /dogs/my`
  - `PATCH /dogs/:id`
- signup에서 강아지 1마리 생성 포함(온보딩 1-step), 이후 `/dogs`로 관리(조회/수정/추가등록)
